...
TEST_CASE("Create, Read, Write")
{
    ...
    SECTION("Creating by reading from file","[size]")
    {
        ...
        Matrix m1 = Matrix("1sizematrix.txt"); // 1 sized matrix
        CHECK(m1.getSize() == 1);


        Matrix m2 = Matrix("2sizematrix.txt");//2 sized matrix
        CHECK(m2.getSize() == 2);


        Matrix m3 = Matrix("5matrix.txt");
        CHECK(m3.getSize() == 5);//5 sized matrix
    }
}
...
TEST_CASE("Multiplying two matrices")
{
    ...
    {
        Matrix a;
        Matrix identity = Matrix("3identitymatrix.txt");
